using Microsoft.AspNetCore.Mvc;
using swi2grupp1WebAPI.MessageHandler;

namespace swi2grupp1WebAPI.Controllers
{    /*
     * Controller f�r REST-Services
     * Alle Aufrufe sind asynchron programmiert
     * GET
     * POST
     */
    [ApiController]
    [Route("[controller]")]
    public class FilmController : ControllerBase
    {


        private readonly ILogger<FilmController> _logger;
        public FilmController(ILogger<FilmController> logger)
        {
            _logger = logger;
        }

        /*
         * GET-Methode
         * Gibt ein Array von WeatherForecast-Objekten
         * kann mit einem Parameter Ort gefiltert werden
         */

        [HttpGet(Name = "GetFilm")]

        public async Task<IEnumerable<Film>> Get(int? id = 0, string? name = "")
        {

            Film[] films = await new MessageHandler.MessageHandler().GetFilmAsync(id, name);
            return films;
        }
        /*
         * POST-Methode - neues Objekt aufnehmen
         * erwartet ein WeatherForecast-JSON-Objekt
         * Dieses wird an den Azure Service Bus zur Weiterverarbeitung gesendet
         */
        [HttpPost(Name = "PostFilm")]
        //public void Post([FromBody]WeatherForecast weatherForecast)
        public async void Post(Film film)
        {
            // Objekt versenden
            int statusCode = await new MessageHandler.MessageHandler().CreateFilmAsync(film);
        }
        /*
        * PUT-Methode - bestehendes Objekt �ndern
        * erwartet ein WeatherForecast-JSON-Objekt
        * Dieses wird an den Azure Service Bus zur Weiterverarbeitung gesendet
        */
        [HttpPut(Name = "PutFilm")]
        public async void Put(Film film)
        {
            // Objekt versenden
            int statusCode = await new MessageHandler.MessageHandler().UpdateFilmAsync(film);
        }
        /*
        * DELETE-Methode - bestehendes Objekt l�schen
         * erwartet ein WeatherForecast-JSON-Objekt
         * Dieses wird an den Azure Service Bus zur Weiterverarbeitung gesendet
        */
        [HttpDelete(Name = "DeleteFilm")]
        public async void Delete(int Id)
        {
            // Objekt versenden
            int statusCode = await new MessageHandler.MessageHandler().DeleteFilmAsync(Id);
        }
    }
}